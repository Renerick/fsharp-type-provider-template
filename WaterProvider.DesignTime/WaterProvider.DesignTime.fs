module WaterProviderImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("WaterProvider.DesignTime", "WaterProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("WaterProvider.DesignTime", "WaterProvider.Runtime")])

    let ns = "WaterProvider"
    let asm = Assembly.GetExecutingAssembly()
    ...
    let createType typeName (count:int) =
        ...
        for i in 1 .. count do 
            ...
            let auxProp = ProvidedProperty("AuxProperty" + string i, typeof<int>, getterCode = fun args -> <@@ i @@>)
            myType.AddMember(auxProp)
